...
# this is supposed to create a list of all the working groups, but is broken - it only scrapes the first 6 lists for some reason. So I've selected the groups to scrape manually, in the wgsAll list above
def getWGs():
...
def keyCheck(messageUrl, keyword):
	...
	if keyword in str(soup):
		result = "<p><a href=\"%s\">%s</a></p>" % (messageUrl, messageUrl)
...
keyword = str(raw_input("Enter one keyword to search: "))
